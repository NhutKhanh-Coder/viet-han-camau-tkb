...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'update_info') {
        ...
        $sdt   = trim($_POST['sdt'] ?? '');
        $st = $db->prepare("UPDATE users SET email=?, sdt=? WHERE id=?");
        $st->bind_param("ssi", $email, $sdt, $uid);
        $st->execute();
    ...
    if ($_POST['action'] === 'upload_avatar') {
        if (isset($_FILES['avatar_file']) && $_FILES['avatar_file']['error'] === 0) {
            ...
            if (in_array($ext, $allowed)) {
                $newFilename = "teacher_avatar_" . $uid . "_" . time() . "." . $ext;
...
// Fetch user & teacher details
$st2 = $db->prepare("SELECT u.*, g.ma_gv, g.khoa FROM users u JOIN giang_vien g ON g.user_id = u.id WHERE u.id=?");
$st2->bind_param("i", $uid);
$st2->execute();
$teacher = $st2->get_result()->fetch_assoc();
$hasFace = !empty($teacher['face_descriptor']);
$db->close();
...
<title>Hồ Sơ Gi giảng viên</title>
...
.prof-avatar-btn:hover { background: var(--accent); opacity: 0.9; }
...
.prof-pill-blue { background: rgba(56, 189, 248, 0.08); color: #38bdf8; padding: 10px; border-radius: 20px; font-size: 12px; font-weight: 700; text-align: center; display: block; text-decoration: none; margin-bottom: 10px; display: flex; align-items: center; justify-content: center; gap: 8px; border: 1px solid rgba(56, 189, 248, 0.12); }
...
.prof-btn-save { background: var(--accent); color: #fff; border: none; padding: 12px 25px; border-radius: var(--r-sm); font-weight: 700; font-size: 13px; cursor: pointer; transition: 0.3s; display: flex; align-items: center; gap: 8px; font-family: 'Outfit', sans-serif; }
...
<?php include '../includes/teacher_nav.php'; ?>
  <div class="page-header">
    <div><h1 class="page-title"><i class="fa-solid fa-id-card" style="color:var(--accent)"></i> Hồ Sơ Cá Nhân</h1></div>
  </div>
  ...
  <div class="profile-grid">
    ...
    <div class="prof-card">
        <div class="prof-avatar-wrap">
            <?php $avatar_url = !empty($teacher['avatar']) ? '/tkb/assets/img/avatars/' . $teacher['avatar'] : '/tkb/assets/img/logo_vkc.jpg'; ?>
        ...
        <div class="prof-name"><?= htmlspecialchars($teacher['ho_ten'] ?? 'Chưa cập nhật') ?></div>
        <div class="prof-msv">Mã GV: <?= htmlspecialchars($teacher['ma_gv'] ?? 'Chưa cập nhật') ?></div>
        
        <div class="prof-pill-blue"><i class="fa-solid fa-chalkboard-user"></i> Khoa: <?= htmlspecialchars($teacher['khoa'] ?? 'Chung') ?></div>
        <div class="prof-pill-gray">GIẢNG VIÊN CHÍNH THỨC</div>
        
        <div class="prof-divider"></div>
        
        <div class="prof-meta-item">
            <div class="prof-meta-label">VAI TRÒ</div>
            <div class="prof-meta-val">Giảng dạy học phần</div>
        </div>
    </div>

    <!-- Right Column: Form & Face -->
    <div class="prof-card">
        <h3 class="prof-title"><i class="fa-solid fa-gears" style="color:var(--accent); margin-right:8px;"></i> Thông Tin & Bảo Mật</h3>
        ...
        <form method="POST" style="margin-bottom: 40px;">
            ...
            <div class="prof-form-row">
                <div class="prof-input-wrap">
                    ...
                    <input type="email" name="email" class="prof-input" placeholder="Nhập email của bạn" value="<?= htmlspecialchars($teacher['email'] ?? '') ?>">
                </div>
                <div class="prof-input-wrap">
                    <label style="font-size:11px; font-weight:700; color:var(--text2); display:block; margin-bottom:8px;">SỐ ĐIỆN THOẠI</label>
                    <i class="fa-solid fa-phone"></i>
                    <input type="text" name="sdt" class="prof-input" placeholder="Nhập số điện thoại" value="<?= htmlspecialchars($teacher['sdt'] ?? '') ?>">
                </div>
            </div>
        ...
        <!-- Đổi Mật Khẩu -->
        <h3 class="prof-title"><i class="fa-solid fa-key" style="color:var(--accent); margin-right:8px;"></i> Đổi Mật Khẩu</h3>
